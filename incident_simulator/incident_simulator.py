...
class IncidentSimulator:
    ...
    def __init__(
        ...
        logger.info(f"IncidentSimulator initialized with {len(house.nodes)} nodes, {len(house.edges)} edges")
    ...
    def _create_incident(
        ...
        logger.debug(f"New incident: {inc_type.value} at {location_type} {location_id}, severity={severity:.2f}")
    ...
    def get_incident_tensors(self) -> Dict[str, torch.Tensor]:
        ...
        node_incident_severity = torch.zeros(len(self.house.nodes), dtype=torch.float32)
        node_incident_type = torch.zeros(len(self.house.nodes), len(IncidentType), dtype=torch.float32)
        
        for idx, node in enumerate(self.house.nodes):
            node_id = id(node)
            incidents = [inc for inc in self.active_incidents if inc.location_id == node_id]
            ...
            if incidents:
                ...
                type_idx = list(IncidentType).index(worst_incident.incident_type)
                node_incident_type[idx, type_idx] = 1.0
        
        edge_incident = torch.zeros(len(self.house.edges), dtype=torch.float32)
        edge_incident_severity = torch.zeros(len(self.house.edges), dtype=torch.float32)
        edge_incident_type = torch.zeros(len(self.house.edges), len(IncidentType), dtype=torch.float32)
        
        for idx, edge in enumerate(self.house.edges):
            edge_id = id(edge)
            incidents = [inc for inc in self.active_incidents if inc.location_id == edge_id]
