...
class BuildingIncidentEnv(gym.Env):
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        action_reward = self.reward_strategy.calculate_action_reward(
            self, parsed_action)

        sim_results = self.simulator.step()

        reward = action_reward + \
            self.reward_strategy.calculate_step_reward(self)
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[Dict] = None) -> Tuple[np.ndarray, Dict]:
        ...
        observation = self.observer.get_observation()
        info = {"status": "reset", "num_nodes": len(
            self.house.nodes), "num_edges": len(self.house.edges)}
        ...
        if self.render_mode == "human":
            ...
            if self.simulator.active_incidents:
                ...
                for inc in self.simulator.active_incidents[:5]:
                    element = self.simulator._get_element(
                        inc.location_id, inc.location_type)
                    element_name = type(
                        element).__name__ if element else "Unknown"
